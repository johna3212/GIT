...
module.exports = {
    config: {
        ...
        usage: "<winners> <duration> <item>",
        category: "owner",                             
        accessableby: "Giveaway makers",
        aliases: []
    },
   ...
   // Argumenten die we later nodig hebben.
    ...
    setTimeout(function () {
        ...
        if (peopleReacted.length == 0) {
            return message.channel.send("No one won!.");
        }
 
        // Tijdelijk kijken we na als er te wienig mensen hebben mee gedaan aan de wedstrijd.
        if (peopleReacted.length < winnerCount) {
            return message.channel.send("There aren't enough people who entered! So the bot just won!");
        }
        ...
        for (var i = 0; i < winners.length; i++) {
            message.channel.send("GG " + winners[i] + ` You won! **${item}**.`);
        }
